...
import { FileDown, Share2, Save, BarChart3, Check, Copy } from 'lucide-react';
...
import type { WizardState } from '@/src/components/v2/types';

interface SimulatorActionsV2Props {
  result: SimulatorV2Result;
  wizardState: WizardState;
  onCompareClick: () => void;
  onScenarioSaved?: () => void;
}

export default function SimulatorActionsV2({ 
  result, 
  wizardState, 
  onCompareClick, 
  onScenarioSaved 
}: SimulatorActionsV2Props) {
  ...
  const handlePDFExport = async () => {
    // Convert V2 result to V1 format for PDF export
    const v1Result = {
      tax: {
        grossAnnual: wizardState.grossIncome,
        netAnnual: result.combinedTax.netAnnual,
        netMonthly: result.combinedTax.netMonthly,
        totalTax: result.combinedTax.totalTax,
        effectiveTaxRate: result.combinedTax.effectiveTaxRate,
        federalTax: result.primaryTax.federalTax,
        provincialTax: result.primaryTax.provincialTax,
        qppContribution: result.primaryTax.qppContribution,
        qpipContribution: result.primaryTax.qpipContribution,
        eiContribution: result.primaryTax.eiContribution,
      },
      city: result.city,
      disposableIncome: result.monthlyBreakdown.disposable,
      monthlyExpenses: result.monthlyBreakdown.totalExpenses,
      savingsRate: result.financialHealth.savingsRate,
      annualDisposableIncome: result.monthlyBreakdown.disposable * 12,
      financialHealth: {
        status: result.financialHealth.status,
        label: result.financialHealth.status === 'excellent' ? 'Excellent' :
               result.financialHealth.status === 'good' ? 'Bon' :
               result.financialHealth.status === 'tight' ? 'Serré' : 'Déficit'
      }
    };

    await exportToPDF({ 
      result: v1Result as any, 
      grossSalary: wizardState.grossIncome, 
      hasPartner: result.household.hasPartner, 
      hasCar: wizardState.transportType === '1-car' || wizardState.transportType === '2-cars'
    });
  };
  ...
  const handleShare = (type: 'email' | 'whatsapp' | 'facebook') => {
    // Convert V2 result to V1 format for sharing
    const v1Result = {
      tax: {
        grossAnnual: wizardState.grossIncome,
        netAnnual: result.combinedTax.netAnnual,
    ...
    if (type === 'email') {
      shareViaEmail({ result: v1Result as any, grossSalary: wizardState.grossIncome });
    } else if (type === 'whatsapp') {
      shareViaWhatsApp({ result: v1Result as any, grossSalary: wizardState.grossIncome });
    } else {
      shareViaFacebook();
    }
